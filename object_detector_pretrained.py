...
#   *Has a size of (1, 100, 1): 1 image in the batch, 100 potential objects in the photo, 1 class index per object.
...
# Print out descriptors for the top k objects detected.
#   NOTE: The model automatically sorts its objects from greatest to least probability.
k = 10
print("top 10 object indices")
print(object_indices[0][:k])
print("top 10 object probabilities")
print(object_probabilities[0][:k])
print("top 10 bounding boxes")
print(bounding_boxes[0][:k])

# Given the top k objects found, print their predicted object class and the probability of the object belonging to the class.
#   NOTE: To extract scalar value from mxnet ndarray, we have to first convert it to a numpy ndarray and then
#   use the item() to get a float scalar. We then convert the float to an int for array indexing the array of labels.
for i in range(10):
    # If objects are no longer detected, the class index of an object will be -1.
...
# Plot the bounding boxes of the objects along with their predicted object class and probability onto the input image.
gcv.utils.viz.plot_bbox(hwc_image, bounding_boxes[0], scores=object_probabilities[0], labels=object_indices[0], class_names=model.classes, thresh=0.2)
plt.show()
